#!/usr/bin/env bash
# Revert ios/Podfile.lock when `pod install` only rewrote the three SPEC CHECKSUM
...
# script). A pod's SPEC CHECKSUM is the SHA1 of its generated podspec, so those three
...
#
# A diff limited to those three lines carries no information — a genuine pod change
# also moves that pod's version line — so it is safe to drop.
set -euo pipefail
