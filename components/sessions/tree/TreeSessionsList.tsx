...
export const TreeSessionsList = React.memo(function TreeSessionsList({
  sessions,
  summaries,
  refreshing,
  onRefresh,
  ...
  const renderSearchItem = useCallback(
    ({ item }: { item: MultiConversation | MultiSession }) => {
      ...
      if (isSession) {
        ...
        return (
          <ConversationListItem
            ...
            title={sessionRowTitle(s, storedNameFor(names, nameOrigins, s.serverId, s.id))}
            path={s.projectPath}
      ...
      return (
        <ConversationListItem
          ...
          title={conversationRowTitle(c, storedNameFor(names, nameOrigins, c.serverId, c.id))}
          path={c.projectPath}
    ...
    [router, servers, activeServerCount, debouncedQuery, names, nameOrigins],
  )
  ...
  const handleSessionPress = useCallback((s: MultiSession) => {
    useNavLockStore.getState().lock()
    router.push(`/session/${s.id}?server=${s.serverId}`)
  }, [router])
    ...
    return (
      <View style={{ marginBottom: 4 }}>
        ...
        {(!collapsible || !sessionsCollapsed) && sessions.map((s) => {
          const { tier } = deriveSessionPresentation(s)
          const serverColor = s.serverId ? servers[s.serverId]?.color : undefined
          return (
            <ConversationListItem
              key={`${s.serverId}::${s.id}`}
              testID={`session-row-${s.id}`}
  ...
  }, [sessions, sessionsCollapsed, setSessionsCollapsed, servers, names, nameOrigins, activeServerCount, handleSessionPress])

  if (selectedDrill && !searchOpen) {
    return (
      <DrillView
        key={selectedDrill.node.fullPath}
      ...
      {showSearch ? (
        searchSections.length === 0 ? (
          ...
          <SectionList
            ...
            {...inset.props}
            contentContainerStyle={[searchStyles.listContent, inset.contentStyle]}
        ...
        <FlatList
          ...
          {...LIST_WINDOW}
          {...inset.props}
          contentContainerStyle={[flatItems.length === 0 ? { flexGrow: 1 } : styles.listContent, inset.contentStyle]}
          ListHeaderComponent={
            <View>
              {ListHeaderComponent}
              {liveSessionsBlock}
