...
describe('useWsStatus', () => {
  it('seeds from the manager and tracks status events for its server', () => {
    __wsTest.setStatus('connected')
    const { result } = renderHook(() => useWsStatus('srv-1'))
  ...
  it('ignores status events for other servers', () => {
    __wsTest.setStatus('connected')
    const { result } = renderHook(() => useWsStatus('srv-1'))
  ...
  it('unsubscribes on unmount', () => {
    const { unmount } = renderHook(() => useWsStatus('srv-1'))
