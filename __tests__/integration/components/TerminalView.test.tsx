...
describe('TerminalView', () => {
  ...
  it('renders terminal-line-row elements when lines are provided', () => {
    renderView()
    const rows = screen.getAllByTestId('terminal-line-row')
    expect(rows.length).toBeGreaterThanOrEqual(2)
  })

  it('renders chat-message-input (the composer is present)', () => {
    renderView()
    expect(screen.getByTestId('chat-message-input')).toBeTruthy()
  })

  it('tapping chat-send-button calls sendInput.mutate with the payload', () => {
    renderView()
