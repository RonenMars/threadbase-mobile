...
2. **The Needs-you qualifier stays, fed by `statusUpdatedAt`.** An earlier draft of this ruling cut it on the claim that no wait-start field exists; that claim was wrong. The streamer (1.91.0) already ships `statusUpdatedAt`, stamped by all three runners on every `status` flip and serialised on REST and WebSocket, so while `status === 'waiting_input'` it is exactly when the wait began. PR 3 declares it on `Session` in `types/api.ts` and renders `Needs you · waiting 2m` from it (`row.waitingFor`). A rehydrated stub carries an old stamp but is never `waiting_input` after rehydrate, so the gate holds. Do not approximate from `elapsedMs`.
...
6. `feat(sessions): state-ordered Now list` — new default view. Shipped: `components/sessions/now/` (eyebrow, Needs-you and Working cards, history rows, grouped-noise rows, the list); the merged Classic list and `ClassicSessionsList` are gone; the FAB is the `+ New session` pill. Ruling 5 applied: `rowDensity`, `rowTitleSource` and `aiGeneratedNames` are deleted, `rowPreviewMode` is the subtitle switch. Deviations recorded here: (a) the `Now | Projects` segmented control and the retirement of the Hub/Tree switch move to PR 9 with the `sessionsLayout` migration, so the non-merged Classic tabs and `LiveSessionsHeader` (Tree) survive until then; (b) history splits into `EARLIER TODAY` and `EARLIER` rather than one label, because older rows carry dates, not clock stamps; with more than one server history groups per server instead, the two live sections stay global; (c) Observed rows sit first in history; (d) **list conversations cannot render Can't resume**: `resumable` / `unavailableReason` exist only on `ConversationDetail`, not on the list shape (`types/api.ts`), so the tier appears on failed sessions only until the streamer exposes those two fields on `/api/conversations`.
7. `feat(sessions): provider marks on non-dominant rows` — lobe-icons + dominance rule; removes the word badges.
8. `refactor(servers): move the view switch out of Filter & Sort` — shipped: the View section is gone (the layout switch stays in Settings until PR 9 ships the segmented control), presets, `SHOW` tier chips with live counts, `AGENT` multi-select, `ACTIVE WITHIN`, `ORDER` as State-then-recent / Recent / Project with a scoped direction toggle, and a sticky footer whose primary action reads `Show N results` and disables at zero. Filtering is **client-side** in `lib/sessionFilters.ts`: the wire only knows three statuses, and an empty status list used to mean "all". A single selected agent still narrows the paged conversations query server-side. `SortBy` is now `state | lastActivity | projectName`; `Created date` and `Sort by status` are gone. The Tree drill E2E flow switches layout through Settings.
9. `feat(sessions): fold Tree into Projects as a path drill` — shipped: `sessionsLayout` is now `'now' | 'projects'` (default `now`); `hydrate` maps persisted `tree` and `hub` to `projects` and `classic` to `now`, with a test per value. The `Now | Projects` segmented control sits under the hub header (`layout-now` / `layout-projects`) and the Settings layout row is gone with its four keys. The drill opens from an open project card's `Browse path ›` footer (`hub-browse-path-<path>`) and reuses `DrillView`; `TreeSessionsList`, `TreeRow`, `ServerRootRow` and `LiveSessionsHeader` are deleted, `sessions:live.headerLive_*` / `headerIdle_*` with them. Deviations: (a) the non-merged `Sessions | History` tabs under `mergeChats = false` survive; retiring `mergeChats` is its own change because `non-merged-conv-loading.yaml` depends on it; (b) the Projects card redesign from `5a` (ACTIVE / RECENT / QUIET eyebrows, path filter field, `N live · last <time>` line, the drill's `FOLDERS` / `HERE · N` labels) is not in this PR; the card keeps today's anatomy with the drill link added. `e2e/feat1_tree_drill_new_session.yaml` keeps its name and now drives the Projects path.
