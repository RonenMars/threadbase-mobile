...
| 5 | The app shipped the **wrong** privacy-policy URL | **High** | **P0** | [#624](https://github.com/RonenMars/threadbase-mobile/issues/624) | The policy lives at `https://threadbase.sh/privacy-policy` (HTTP 200). The app and the App Store Connect listing pointed at `/privacy`, which 404s. Apple requires a working Privacy Policy URL, so this fails App Review on the first click, and it was already a dead link for TestFlight users from `app/settings.tsx` and `app/help-feedback.tsx`. **Fixed in-app 2026-08-10**; the App Store Connect and Play Console fields still need updating by hand |
