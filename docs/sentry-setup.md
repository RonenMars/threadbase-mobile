...
- **Local, or a GitHub-hosted runner.** Under `CI`, Expo skips the Metro cache reset that release builds rely on, and the transform cache is not keyed on `EXPO_PUBLIC_*` values, so an enforced `services/sentry.ts` could be reused by a later store build on the same runner. The script refuses to run under `CI` unless `RUNNER_ENVIRONMENT` is `github-hosted`, whose VMs are discarded after the job; that is how the `QA (Firebase)` workflow (`.github/workflows/qa.yml`) runs it.
