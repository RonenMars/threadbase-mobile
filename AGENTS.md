...
- Maestro flows and fixtures live in `e2e/`; `npm run test:e2e:mock` is the authoritative mock suite. Add new flows to the relevant `test:e2e:*` script and prefer `testID` over unstable visible text. The GitHub `E2E` workflow shards that same list across parallel jobs after one Release build per platform; do not duplicate the flow list in the workflow YAML.
