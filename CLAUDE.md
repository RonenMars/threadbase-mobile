...
CI (`.github/workflows/e2e.yml`) does not run that script as one job. It builds the Release binary once per platform, then fans the same flow list across parallel Maestro shards (`e2e/mock-suite-shards.js`: 3 Android emulators, 3 iOS simulators). A `flows=` dispatch stays on a single shard. Local `npm run test:e2e:mock` is unchanged.
