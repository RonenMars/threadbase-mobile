...
Repo-scoped skills for agents that read `.agents/skills`. Each subdirectory is one skill, with its instructions in `SKILL.md`.

**Nothing registers them.** Discovery walks every `.agents/skills/<name>/SKILL.md` in the project
automatically — there is no list in `AGENTS.md`, no settings entry, no build step. Adding a directory
...
Where `AGENTS.md` mentions a skill, it is giving repo context at the place that context is needed, not
registering anything. Do not add a skill to `AGENTS.md` just to make it visible; it already is.
...
- Cite repo docs by relative path rather than restating them — `AGENTS.md` and `docs/` stay canonical.
...
Personal skills live in `~/.agents/skills/` and are discovered the same way; put a skill here only when
