...
1. **Pre-ship checks** — shells out to `./scripts/git-sync-check.sh`. Fails fast if:
   ...
   See [`.Codex/skills/_shared/pre-ship-checks.md`](../_shared/pre-ship-checks.md) for the rationale behind these checks. They mirror the gates `./scripts/ship-ios.sh` runs.
